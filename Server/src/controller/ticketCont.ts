...
export const createTicket = async (req: Request, res: Response) => {
  try {
    // Create ticket
    const { event, availableQty, location, description, image, price } =
      req.body;
      ...
      try {
        ...
        for (const selectedUser of selectedUsers) {
          ...
          const email = {
            ...
            text: `Congratulations! You have been selected to purchase a ticket for ${ticket.event}. with reservation number ${selectedUser.reservationId} Please follow this link to purchase the ticket before it expires: http://example.com/purchase/${selectedUser.id}`,
          };
